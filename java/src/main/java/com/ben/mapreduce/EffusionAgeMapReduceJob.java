...
/**
 * Reads a CSV file from HDFS, maps and reduces it,
 * and places the results in a Cassandra database.
 * 
 * The CSV file in this example is about EKG data 
 * from the UCI machine learning dataset repository:
 *  https://archive.ics.uci.edu/ml/datasets/Echocardiogram
 * 
 * Typically run as:
 *  hadoop -jar ThisJar.jar
 * 
 * Command line args:
 * 	arg[0] = inputPath
 *  arg[1] = outputPath
...
public class EffusionAgeMapReduceJob {
	 ...
	 * Logger
	 */
	private static final Logger LOG = LoggerFactory.getLogger(EffusionAgeMapReduceJob.class);
	...
	public static class TokenizerMapper extends Mapper<Object, Text, IntWritable, DoubleWritable> {
		...
		public void map(Object key, Text value, Context context)
			...
			try {
	    		...
	    		context.write(new IntWritable(effusion), new DoubleWritable(age));
  ...
  /**
   * Reduce the results by the average age of 
   * the effusion patients, and the average
   * age of the non-effusion patients.
   * 
  ...
  public static class IntSumReducer extends Reducer<IntWritable,DoubleWritable,IntWritable,DoubleWritable> {	  
	  ...
	  public void reduce(IntWritable key, Iterable<DoubleWritable> values, Context context)
		throws IOException, InterruptedException {
		  ...
		  PreparedStatement stmt = session.prepare(
				  "INSERT INTO " + OUTPUT_COLUMN_FAMILY + " (effusion, avg_age) VALUES (?, ?);"
				  );
  	...
  	public static void main(String[] args) throws Exception {
	    ...
	    SlicePredicate predicate = new SlicePredicate().setColumn_names(colNames);
	    */
